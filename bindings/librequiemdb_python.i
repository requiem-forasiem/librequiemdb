...
void swig_python_raise_exception(int error, const char *strerror)
{
        ...
        module = PyImport_ImportModule("requiemdb");
        exception_class = PyObject_GetAttrString(module, "RequiemDBError");
...
%typemap(in, numinputs=0) requiemdb_result_values_t **result2 ($*1_type tmp) {
        $1 = ($1_ltype) &tmp;
};


%typemap(in) requiemdb_result_values_t ** {
        if ( $input == Py_None )
                return NULL;
...
%typemap(argout) requiemdb_result_values_t **result2 {
	long size;
...
%inline %{
int requiemdb_get_values2(requiemdb_t *db, requiemdb_path_selection_t *path_selection,
                         idmef_criteria_t *criteria, requiem_bool_t distinct, int limit, int offset,
                         requiemdb_result_values_t **result2) 
{
        return requiemdb_get_values(db, path_selection, criteria, distinct, limit, offset, result2);
}
%}


%typemap(in, numinputs=0) requiemdb_result_idents_t **result2 ($*1_type tmp) {
        $1 = ($1_ltype) &tmp;
};


%typemap(in) requiemdb_result_idents_t ** {
        if ( $input == Py_None )
                return NULL;
...
%typemap(argout) requiemdb_result_idents_t **result2 {
	long size;
...
%inline %{
int requiemdb_get_alert_idents2(requiemdb_t *db, idmef_criteria_t *criteria,
                               int limit, int offset,
                               requiemdb_result_idents_order_t order,
                               requiemdb_result_idents_t **result2) 
{
        return requiemdb_get_alert_idents(db, criteria, limit, offset, order, result2);
}

int requiemdb_get_heartbeat_idents2(requiemdb_t *db, idmef_criteria_t *criteria,
                                   int limit, int offset,
                                   requiemdb_result_idents_order_t order,
                                   requiemdb_result_idents_t **result2) 
{
        return requiemdb_get_heartbeat_idents(db, criteria, limit, offset, order, result2);
}
%}
...
%typemap(in, numinputs=0) (char *errbuf, size_t size) (char tmp[REQUIEMDB_ERRBUF_SIZE]) {
	$1 = tmp;
	$2 = sizeof(tmp);
};
...
%typemap(in, numinputs=0) requiem_string_t *output {
	int ret;
...
class RequiemDBError(requiem.RequiemError):
	...
	if self._strerror:
        ...
        return requiemdb_strerror(self.errno) or ""
%}

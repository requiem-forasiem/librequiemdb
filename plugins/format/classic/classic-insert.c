...
static inline const char *get_string(requiem_string_t *string)
{
        const char *s;
        ...
        s = requiem_string_get_string(string);

        return s ? s : "";
}


static inline int get_data(requiemdb_sql_t *sql, idmef_data_t *data, char **output)
{
        int ret;
	...
	switch ( idmef_data_get_type(data) ) {
        ...
        case IDMEF_DATA_TYPE_BYTE_STRING:
                return requiemdb_sql_escape_binary(sql, idmef_data_get_data(data), idmef_data_get_len(data), output);
                
        case IDMEF_DATA_TYPE_CHAR_STRING:
                return requiemdb_sql_escape_binary(sql, idmef_data_get_data(data), idmef_data_get_len(data) - 1, output);

        case IDMEF_DATA_TYPE_CHAR:
		return requiemdb_sql_escape_binary(sql, idmef_data_get_data(data), 1, output);
	...
	default:
		...
		ret = requiemdb_sql_escape_binary(sql, (const unsigned char *) requiem_string_get_string(string),
                                                  requiem_string_get_len(string), output);
		requiem_string_destroy(string);
...
get_optional_integer(uint64, uint64_t, "%" REQUIEM_PRIu64)
get_optional_integer(float, float, "%f")


static int insert_address(requiemdb_sql_t *sql,
			  ...
			  idmef_address_t *address)
{
        ...
        ret = requiemdb_sql_escape(sql, idmef_address_category_to_string(idmef_address_get_category(address)), &category);
        if ( ret < 0 )
                return ret;

        ret = requiemdb_sql_escape(sql, get_string(idmef_address_get_ident(address)), &ident);
        if ( ret < 0 ) {
                free(category);
                return ret;
        }
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_address_get_address(address)), &addr);
        if ( ret < 0 ) {
                free(ident);
                free(category);
                return ret;
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_address_get_netmask(address)), &netmask);
        if ( ret < 0 ) {
                free(ident);
                free(addr);
                free(category);
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_address_get_vlan_name(address)), &vlan_name);
        if ( ret < 0 ) {
                free(ident);
                free(addr);
                free(netmask);
...
static int insert_node(requiemdb_sql_t *sql,
                       ...
                       idmef_node_t *node)
{
	...
	int index;
        ...
        ret = requiemdb_sql_escape(sql, idmef_node_category_to_string(idmef_node_get_category(node)), &category);
        if ( ret < 0 )
                return ret;

        ret = requiemdb_sql_escape(sql, get_string(idmef_node_get_ident(node)), &ident);
        if ( ret < 0 ) {
                free(category);
                return ret;
        }
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_node_get_name(node)), &name);
        if ( ret < 0 ) {
                free(ident);
                free(category);
                return ret;
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_node_get_location(node)), &location);
        if ( ret < 0 ) {
                free(name);
                free(ident);
                free(category);
...
static int insert_user_id(requiemdb_sql_t *sql,
			  ...
			  idmef_user_id_t *user_id) 
{
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_user_id_get_ident(user_id)), &ident);
        if ( ret < 0 )
                return ret;

        ret = requiemdb_sql_escape(sql, get_string(idmef_user_id_get_tty(user_id)), &tty);
        if ( ret < 0 ) {
                free(ident);
                return ret;
        }
        
        ret = requiemdb_sql_escape(sql, idmef_user_id_type_to_string(idmef_user_id_get_type(user_id)), &type);
        if ( ret < 0 ) {
                free(tty);
                free(ident);
                return ret;
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_user_id_get_name(user_id)), &name);
        if ( ret < 0 ) {
                free(tty);
                free(type);
                free(ident);
...
static int insert_user(requiemdb_sql_t *sql, char parent_type, uint64_t message_ident, int parent_index,
		       idmef_user_t *user) 
{
        char *category, *ident;
	...
	int index;
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_user_get_ident(user)), &ident);
        if ( ret < 0 )
                return ret;
        
        ret = requiemdb_sql_escape(sql, idmef_user_category_to_string(idmef_user_get_category(user)), &category);
        if ( ret < 0 ) {
                free(ident);
                return ret;
        }
        
        ret = requiemdb_sql_insert(sql, "Requiem_User",
				   "_parent_type, _message_ident, _parent0_index, "
				   "ident, category",
				   "'%c', %" REQUIEM_PRIu64 ", %d, %s, %s",
				   parent_type, message_ident, parent_index,
				   ident, category);
...
static int insert_process_arg(requiemdb_sql_t *sql,
			      ...
			      requiem_string_t *arg)
{
	...
	ret = requiemdb_sql_escape(sql, get_string(arg), &tmp);
	if ( ret < 0 )
		return ret;
                
	ret = requiemdb_sql_insert(sql, "Requiem_ProcessArg",
				   "_parent_type, _message_ident, _parent0_index, _index, arg",
				   "'%c', %" REQUIEM_PRIu64 ", %d, %d, %s",
...
static int insert_process_env(requiemdb_sql_t *sql,
			      char parent_type, uint64_t message_ident, int parent_index, int env_index,
			      requiem_string_t *env)
{
	char *tmp;
	int ret;

	ret = requiemdb_sql_escape(sql, get_string(env), &tmp);
	if ( ret < 0 )
		return ret;
                
	ret = requiemdb_sql_insert(sql, "Requiem_ProcessEnv",
...
static int insert_process(requiemdb_sql_t *sql, char parent_type, uint64_t message_ident, int parent_index,
                          idmef_process_t *process) 
{
        requiem_string_t *process_arg, *last_process_arg;
        requiem_string_t *process_env, *last_process_env;
	...
	int index;
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_process_get_ident(process)), &ident);
        if ( ret < 0 )
                return ret;
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_process_get_name(process)), &name);
        if ( ret < 0 ) {
                free(ident);
                return ret;
        }
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_process_get_path(process)), &path);
        if ( ret < 0 ) {
                free(ident);
                free(name);
                return ret;
	...
	get_optional_uint32(pid, sizeof(pid), idmef_process_get_pid(process));
        ...
        ret = requiemdb_sql_insert(sql, "Requiem_Process",
...
static int insert_snmp_service(requiemdb_sql_t *sql, char parent_type, uint64_t message_ident, int parent_index,
                               idmef_snmp_service_t *snmp_service) 
{
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_snmp_service_get_oid(snmp_service)), &oid);
	if ( ret < 0 )
		goto error;
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_snmp_service_get_security_name(snmp_service)), &security_name);
	if ( ret < 0 )
		goto error;

        get_optional_uint32(sel, sizeof(sel), idmef_snmp_service_get_security_level(snmp_service));

        ret = requiemdb_sql_escape(sql, get_string(idmef_snmp_service_get_context_name(snmp_service)), &context_name);
	if ( ret < 0 )
		goto error;

        ret = requiemdb_sql_escape(sql, get_string(idmef_snmp_service_get_context_engine_id(snmp_service)), &context_engine_id);
	if ( ret < 0 )
		goto error;
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_snmp_service_get_command(snmp_service)), &command);
	if ( ret < 0 )
		goto error;
        
        ret = requiemdb_sql_insert(sql, "Requiem_SnmpService",
				   ...
				   "'%c', %" REQUIEM_PRIu64 ", %d, %s, %s, %s, %s, %s, %s, %s, %s", 
...
static int insert_web_service_arg(requiemdb_sql_t *sql,
				  char parent_type, uint64_t message_ident, int parent_index, int arg_index,
				  requiem_string_t *arg)
{
	char *tmp;
	...
	ret = requiemdb_sql_insert(sql, "Requiem_WebServiceArg",
				   "_parent_type, _message_ident, _parent0_index, _index, arg",
				   "'%c', %" REQUIEM_PRIu64 ", %d, %d, %s",
				   parent_type, message_ident, parent_index, arg_index,
				   tmp);
...
static int insert_web_service(requiemdb_sql_t *sql,
			      char parent_type, uint64_t message_ident, int parent_index,
			      idmef_web_service_t *web_service) 
{
	requiem_string_t *web_service_arg, *last_web_service_arg;
        char *url, *cgi, *method;
	int index = 0;
        int ret;
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_web_service_get_url(web_service)), &url);
        if ( ret < 0 )
                return ret;
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_web_service_get_cgi(web_service)), &cgi);
        if ( ret < 0 ) {
                free(url);
                return -1;
        }
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_web_service_get_http_method(web_service)), &method);
        if ( ret < 0 ) {
                free(url);
                free(cgi);
                return ret;
        ...
        ret = requiemdb_sql_insert(sql,
				   "Requiem_WebService",
				   "_parent_type, _message_ident, _parent0_index, "
				   "url, cgi, http_method",
				   "'%c', %" REQUIEM_PRIu64 ", %d, %s, %s, %s",
				   parent_type, message_ident, parent_index,
				   url, cgi, method);
...
static int insert_service(requiemdb_sql_t *sql, char parent_type, uint64_t message_ident, int parent_index,
                          idmef_service_t *service) 
{
	...
	get_optional_uint8(ip_version, sizeof(ip_version), idmef_service_get_ip_version(service));
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_service_get_ident(service)), &ident);
        if ( ret < 0 )
		goto error;
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_service_get_name(service)), &name);
        if ( ret < 0 )
		goto error;
	...
	get_optional_uint8(iana_protocol_number, sizeof(iana_protocol_number), idmef_service_get_iana_protocol_number(service));
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_service_get_iana_protocol_name(service)), &iana_protocol_name);
        if ( ret < 0 )
		goto error;

        ret = requiemdb_sql_escape(sql, get_string(idmef_service_get_portlist(service)), &portlist);
        if ( ret < 0 )
		goto error;

        ret = requiemdb_sql_escape(sql, get_string(idmef_service_get_protocol(service)), &protocol);
        if ( ret < 0 )
		goto error;

        ret = requiemdb_sql_insert(sql, "Requiem_Service",
				   "_parent_type, _message_ident, _parent0_index, "
...
static int insert_inode(requiemdb_sql_t *sql,
			uint64_t message_ident, int target_index, int file_index,
			idmef_inode_t *inode) 
{
        char ctime[REQUIEMDB_SQL_TIMESTAMP_STRING_SIZE], ctime_gmtoff[16];
	char number[16], major_device[16], minor_device[16], c_major_device[16], c_minor_device[16];
        ...
        if ( requiemdb_sql_time_to_timestamp(sql, idmef_inode_get_change_time(inode),
	...
	get_optional_uint32(c_minor_device, sizeof(c_minor_device), idmef_inode_get_c_minor_device(inode));
        ...
        ret = requiemdb_sql_insert(sql, "Requiem_Inode",
				   ...
				   "c_minor_device",
				   "%" REQUIEM_PRIu64 ", %d, %d, %s, %s, %s, %s, %s, %s, %s",
...
static int insert_linkage(requiemdb_sql_t *sql, uint64_t message_ident, int target_index, int file_index, int index,
                          idmef_linkage_t *linkage) 
{
        ...
        ret = requiemdb_sql_escape(sql, idmef_linkage_category_to_string(idmef_linkage_get_category(linkage)), &category);
        if ( ret < 0 )
                return ret;

        ret = requiemdb_sql_escape(sql, get_string(idmef_linkage_get_name(linkage)), &name);
        if ( ret < 0 ) {
                free(category);
                return ret;
        }

        ret = requiemdb_sql_escape(sql, get_string(idmef_linkage_get_path(linkage)), &path);
        if ( ret < 0 ) {
                free(name);
                free(category);
                return ret;
        ...
        ret = requiemdb_sql_insert(sql, "Requiem_Linkage",
				   "_message_ident, _parent0_index, _parent1_index, _index, category, name, path",
				   "%" REQUIEM_PRIu64 ", %d, %d, %d, %s, %s, %s",
...
static int insert_file_access_permission(requiemdb_sql_t *sql,
					 uint64_t message_ident, int target_index, int file_index, int file_access_index, int perm_index,
					 requiem_string_t *perm)
{
	char *tmp;
	int ret;

	ret = requiemdb_sql_escape(sql, get_string(perm), &tmp);
	if ( ret < 0 )
		return ret;

	ret = requiemdb_sql_insert(sql, "Requiem_FileAccess_Permission",
				   "_message_ident, _parent0_index, _parent1_index, _parent2_index, _index, permission",
				   "%" REQUIEM_PRIu64 ", %d, %d, %d, %d, %s",
...
static int insert_file_access(requiemdb_sql_t *sql,
			      uint64_t message_ident, int target_index, int file_index, int file_access_index,
                              idmef_file_access_t *file_access)
{
	requiem_string_t *file_access_permission, *last_file_access_permission;
	int index;
        int ret;
        ...
        ret = requiemdb_sql_insert(sql, "Requiem_FileAccess", "_message_ident, _parent0_index, _parent1_index, _index",
				   "%" REQUIEM_PRIu64 ", %d, %d, %d", message_ident, target_index, file_index, file_access_index);
	if ( ret < 0 )
                return ret;
...
static int insert_checksum(requiemdb_sql_t *sql,
			   ...
			   idmef_checksum_t *checksum)
{
	...
	ret = requiemdb_sql_escape(sql, get_string(idmef_checksum_get_value(checksum)), &value);
	if ( ret < 0 )
		return ret;

	ret = requiemdb_sql_escape(sql, get_string(idmef_checksum_get_key(checksum)), &key);
	if ( ret < 0 )
		goto error;

	ret = requiemdb_sql_escape(sql, idmef_checksum_algorithm_to_string(idmef_checksum_get_algorithm(checksum)), &algorithm);
        if ( ret < 0 )
                goto error;

	ret = requiemdb_sql_insert(sql, "Requiem_Checksum",
...
static int insert_file(requiemdb_sql_t *sql, uint64_t message_ident, int target_index, int file_index,
                       idmef_file_t *file) 
{
        int ret = -1;
	...
	int index;
        ...
        char ctime[REQUIEMDB_SQL_TIMESTAMP_STRING_SIZE], ctime_gmtoff[16];
        char mtime[REQUIEMDB_SQL_TIMESTAMP_STRING_SIZE], mtime_gmtoff[16];
        char atime[REQUIEMDB_SQL_TIMESTAMP_STRING_SIZE], atime_gmtoff[16];

        ret = requiemdb_sql_time_to_timestamp(sql, idmef_file_get_create_time(file),
					      ctime, sizeof (ctime), ctime_gmtoff, sizeof (ctime_gmtoff), NULL, 0);
	if ( ret < 0 )
                return ret;

        ret = requiemdb_sql_time_to_timestamp(sql, idmef_file_get_modify_time(file),
					      mtime, sizeof (mtime), mtime_gmtoff, sizeof (mtime_gmtoff), NULL, 0);
	if ( ret < 0 )
                return ret;

        ret = requiemdb_sql_time_to_timestamp(sql, idmef_file_get_access_time(file),
					      atime, sizeof(atime), atime_gmtoff, sizeof (atime_gmtoff), NULL, 0);
	if ( ret < 0 )
		return ret;

        ret = requiemdb_sql_escape(sql, idmef_file_category_to_string(idmef_file_get_category(file)), &category);
	if ( ret < 0 )
		return ret;

        ret = requiemdb_sql_escape(sql, get_string(idmef_file_get_ident(file)), &ident);
	if ( ret < 0 )
		goto error;
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_file_get_name(file)), &name);
	if ( ret < 0 )
		goto error;

        ret = requiemdb_sql_escape(sql, get_string(idmef_file_get_path(file)), &path);
        if ( ret < 0 )
		goto error;

        ret = requiemdb_sql_escape(sql, get_string(idmef_file_get_file_type(file)), &file_type);
        if ( ret < 0 )
		goto error;
	...
	ret = requiemdb_sql_escape(sql, get_optional_enum((int *) idmef_file_get_fstype(file),
							  (char *(*)(int)) idmef_file_fstype_to_string), &fstype);
	if ( ret < 0 )
		goto error;
	
        ret = requiemdb_sql_insert(sql, "Requiem_File", "_message_ident, _parent0_index, _index, ident, category, name, path, "
				   ...
				   "%" REQUIEM_PRIu64 ", %d, %d, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s",
...
static int insert_source(requiemdb_sql_t *sql, uint64_t message_ident, int index, idmef_source_t *source)
{
        int ret;
        char *interface, *spoofed, *ident;
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_source_get_ident(source)), &ident);
	if ( ret < 0 )
		return ret;

        ret = requiemdb_sql_escape(sql, idmef_source_spoofed_to_string(idmef_source_get_spoofed(source)), &spoofed);
	if ( ret < 0 ) {
                free(ident);
		return ret;
        }
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_source_get_interface(source)), &interface);
        if ( ret < 0 ) {
                free(ident);
                free(spoofed);
                return ret;
        ...
        ret = requiemdb_sql_insert(sql, "Requiem_Source",
				   "_message_ident, _index, ident, spoofed, interface",
				   "%" REQUIEM_PRIu64 ", %d, %s, %s, %s",
				   message_ident, index,
...
static int insert_target(requiemdb_sql_t *sql, uint64_t message_ident, int target_index, idmef_target_t *target)
{
        int ret;
	...
	int index;
        ...
        ret = requiemdb_sql_escape(sql, idmef_target_decoy_to_string(idmef_target_get_decoy(target)), &decoy);
        if ( ret < 0 )
                return ret;

        ret = requiemdb_sql_escape(sql, get_string(idmef_target_get_ident(target)), &ident);
        if ( ret < 0 ) {
                free(decoy);
                return -2;
        }
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_target_get_interface(target)), &interface);
        if ( ret < 0 ) {
                free(ident);
                free(decoy);
                return -2;
        ...
        ret = requiemdb_sql_insert(sql, "Requiem_Target",
...
static int insert_analyzer(requiemdb_sql_t *sql,
			   ...
			   idmef_analyzer_t *analyzer) 
{
		...
		*ostype = NULL, *osversion = NULL, *analyzerid = NULL;
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_analyzer_get_analyzerid(analyzer)), &analyzerid);
        if ( ret < 0 )
                return ret;
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_analyzer_get_class(analyzer)), &class);
        if ( ret < 0 )
                goto error;
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_analyzer_get_name(analyzer)), &name);
        if ( ret < 0 )
                goto error;
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_analyzer_get_model(analyzer)), &model);
        if ( ret < 0 )
		goto error;
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_analyzer_get_version(analyzer)), &version);
        if ( ret < 0 )
		goto error;
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_analyzer_get_manufacturer(analyzer)), &manufacturer);
        if ( ret < 0 )
		goto error;

        ret = requiemdb_sql_escape(sql, get_string(idmef_analyzer_get_ostype(analyzer)), &ostype);
        if ( ret < 0 )
		goto error;

        ret = requiemdb_sql_escape(sql, get_string(idmef_analyzer_get_osversion(analyzer)), &osversion);
        if ( ret < 0 )
		goto error;
	
        ret = requiemdb_sql_insert(sql, "Requiem_Analyzer",
...
static int insert_reference(requiemdb_sql_t *sql,
			    ...
			    idmef_reference_t *reference)
{
	...
	int ret = -1;
        ...
        ret = requiemdb_sql_escape(sql, idmef_reference_origin_to_string(idmef_reference_get_origin(reference)), &origin);
        if ( ret < 0 )
                return ret;

        ret = requiemdb_sql_escape(sql, get_string(idmef_reference_get_url(reference)), &url);
        if ( ret < 0 )
		goto error;

        ret = requiemdb_sql_escape(sql, get_string(idmef_reference_get_name(reference)), &name);
        if ( ret < 0 )
		goto error;

	ret = requiemdb_sql_escape(sql, get_string(idmef_reference_get_meaning(reference)), &meaning);
	if ( ret < 0 )
		goto error;

	ret = requiemdb_sql_insert(sql, "Requiem_Reference", "_message_ident, _index, origin, name, url, meaning",
				   "%" REQUIEM_PRIu64 ", %d, %s, %s, %s, %s",
...
static int insert_classification(requiemdb_sql_t *sql, uint64_t message_ident, idmef_classification_t *classification) 
{
        char *text, *ident;
	...
	ret = requiemdb_sql_escape(sql, get_string(idmef_classification_get_ident(classification)), &ident);
	if ( ret < 0 )
		return ret;

        ret = requiemdb_sql_escape(sql, get_string(idmef_classification_get_text(classification)), &text);
	if ( ret < 0 ) {
                free(ident);
		return ret;
        }
        
        ret = requiemdb_sql_insert(sql, "Requiem_Classification",
...
static int insert_additional_data(requiemdb_sql_t *sql, 
				  ...
				  idmef_additional_data_t *additional_data) 
{
        ...
        ret = requiemdb_sql_escape(sql, idmef_additional_data_type_to_string(idmef_additional_data_get_type(additional_data)), &type);
        if ( ret < 0 )
                return ret;
        
        ret = requiemdb_sql_escape(sql, get_string(idmef_additional_data_get_meaning(additional_data)), &meaning);
        if ( ret < 0 ) {
                free(type);
                return ret;
        }
	...
	if ( ret < 0 ) {
        ...
        ret = requiemdb_sql_insert(sql, "Requiem_AdditionalData",
...
static int insert_createtime(requiemdb_sql_t *sql, char parent_type, uint64_t message_ident, idmef_time_t *time) 
{
        char utc_time[REQUIEMDB_SQL_TIMESTAMP_STRING_SIZE];
	char utc_time_usec[16];
	char utc_time_gmtoff[16];
        int ret;

        ret = requiemdb_sql_time_to_timestamp(sql, time, utc_time, sizeof (utc_time), 
					      utc_time_gmtoff, sizeof (utc_time_gmtoff),
					      utc_time_usec, sizeof (utc_time_usec));
	if ( ret < 0 )
		return ret;

        return requiemdb_sql_insert(sql, "Requiem_CreateTime", "_parent_type, _message_ident, time, gmtoff, usec", 
				    "'%c', %" REQUIEM_PRIu64 ", %s, %s, %s",
				    parent_type, message_ident, utc_time, utc_time_gmtoff, utc_time_usec);
}



static int insert_detecttime(requiemdb_sql_t *sql, uint64_t message_ident, idmef_time_t *time) 
{        
        char utc_time[REQUIEMDB_SQL_TIMESTAMP_STRING_SIZE];
	char utc_time_usec[16];
	char utc_time_gmtoff[16];
	...
	if ( ret < 0 )
        ...
        return requiemdb_sql_insert(sql, "Requiem_DetectTime", "_message_ident, time, gmtoff, usec",
				    "%" REQUIEM_PRIu64 ", %s, %s, %s", message_ident, utc_time, utc_time_gmtoff, utc_time_usec);
}



static int insert_analyzertime(requiemdb_sql_t *sql, char parent_type, uint64_t message_ident, idmef_time_t *time) 
{
        char utc_time[REQUIEMDB_SQL_TIMESTAMP_STRING_SIZE];
	char utc_time_usec[16];
	char utc_time_gmtoff[16];
	...
	if ( ret < 0 )
        ...
        return requiemdb_sql_insert(sql, "Requiem_AnalyzerTime", "_parent_type, _message_ident, time, gmtoff, usec",
				    "'%c', %" REQUIEM_PRIu64 ", %s, %s, %s",
				    parent_type, message_ident, utc_time, utc_time_gmtoff, utc_time_usec);
}
...
static int insert_impact(requiemdb_sql_t *sql, uint64_t message_ident, idmef_impact_t *impact) 
{
        int ret = -1;
        ...
        ret = requiemdb_sql_escape(sql, get_optional_enum((int *) idmef_impact_get_completion(impact),
							  (char *(*)(int)) idmef_impact_completion_to_string), &completion);
        if ( ret < 0 )
		return ret;

        ret = requiemdb_sql_escape(sql, idmef_impact_type_to_string(idmef_impact_get_type(impact)), &type);
        if ( ret < 0 ) 
                goto error;

        ret = requiemdb_sql_escape(sql, get_optional_enum((int *) idmef_impact_get_severity(impact),
							  (char *(*)(int))idmef_impact_severity_to_string), &severity);
        if ( ret < 0 )
		goto error;

        ret = requiemdb_sql_escape(sql, get_string(idmef_impact_get_description(impact)), &description);
        if ( ret < 0 )
		goto error;

        ret = requiemdb_sql_insert(sql, "Requiem_Impact", "_message_ident, severity, completion, type, description", 
				   "%" REQUIEM_PRIu64 ", %s, %s, %s, %s", 
...
static int insert_action(requiemdb_sql_t *sql, uint64_t message_ident, int action_index, idmef_action_t *action)
{
        int ret;
        char *category, *description;

	ret = requiemdb_sql_escape(sql, idmef_action_category_to_string(idmef_action_get_category(action)), &category);
        if ( ret < 0 )
                return ret;

        ret = requiemdb_sql_escape(sql, get_string(idmef_action_get_description(action)), &description);
        if ( ret < 0 ) {
                free(category);
                return ret;
        }

        ret = requiemdb_sql_insert(sql, "Requiem_Action",
				   "_message_ident, _index, category, description",
				   "%" REQUIEM_PRIu64 ", %d, %s, %s", 
...
static int insert_confidence(requiemdb_sql_t *sql, uint64_t message_ident, idmef_confidence_t *confidence) 
{
        int ret;
        char *rating;
        ...
        ret = requiemdb_sql_escape(sql, idmef_confidence_rating_to_string(idmef_confidence_get_rating(confidence)), &rating);
        if ( ret < 0 )
                return ret;

        ret = requiemdb_sql_insert(sql, "Requiem_Confidence", "_message_ident, rating, confidence",
				   "%" REQUIEM_PRIu64 ", %s, %f", message_ident, rating, idmef_confidence_get_confidence(confidence));
...
static int insert_assessment(requiemdb_sql_t *sql, uint64_t message_ident, idmef_assessment_t *assessment) 
{
	...
	int ret;
        ...
        ret = requiemdb_sql_insert(sql, "Requiem_Assessment", "_message_ident", "%" REQUIEM_PRIu64, message_ident);
	if ( ret < 0 )
                return ret;
...
static int insert_overflow_alert(requiemdb_sql_t *sql, uint64_t message_ident, idmef_overflow_alert_t *overflow_alert) 
{
        char *program, *buffer, size[16];
        int ret;

        ret = requiemdb_sql_escape(sql, get_string(idmef_overflow_alert_get_program(overflow_alert)), &program);
	if ( ret < 0 )
		return ret;
	...
	get_optional_uint32(size, sizeof(size), idmef_overflow_alert_get_size(overflow_alert));
        ...
        ret = requiemdb_sql_insert(sql, "Requiem_OverflowAlert", "_message_ident, program, size, buffer",
				   "%" REQUIEM_PRIu64 ", %s, %s, %s", 
...
static int insert_alertident(requiemdb_sql_t *sql,
			     ...
			     idmef_alertident_t *alertident)
{
	int ret;
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_alertident_get_analyzerid(alertident)), &analyzerid);
        if ( ret < 0 )
                return ret;

        ret = requiemdb_sql_escape(sql, get_string(idmef_alertident_get_alertident(alertident)), &messageid);
        if ( ret < 0 ) {
                free(analyzerid);
                return ret;
        }
        
	ret = requiemdb_sql_insert(sql, "Requiem_Alertident",
...
static int insert_tool_alert(requiemdb_sql_t *sql, uint64_t message_ident, idmef_tool_alert_t *tool_alert) 
{
        char *name, *command;
	...
	int index;
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_tool_alert_get_name(tool_alert)), &name);
        if ( ret < 0 )
                return ret;

        ret = requiemdb_sql_escape(sql, get_string(idmef_tool_alert_get_command(tool_alert)), &command);
        if ( ret < 0 ) {
                free(name);
                return ret;
        }

        ret = requiemdb_sql_insert(sql, "Requiem_ToolAlert", "_message_ident, name, command",
				   "%" REQUIEM_PRIu64 ", %s, %s",
...
static int insert_correlation_alert(requiemdb_sql_t *sql, uint64_t message_ident,
				    idmef_correlation_alert_t *correlation_alert) 
{
	...
	int index;
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_correlation_alert_get_name(correlation_alert)), &name);
	if ( ret < 0 )
		return ret;

        ret = requiemdb_sql_insert(sql, "Requiem_CorrelationAlert", "_message_ident, name",
				   "%" REQUIEM_PRIu64 ", %s", message_ident, name);
...
static int get_last_insert_ident(requiemdb_sql_t *sql, const char *table_name, uint64_t *result)
{
	...
	int ret;
        ...
        ret = requiemdb_sql_query_sprintf(sql, &table, "SELECT max(_ident) FROM %s;", table_name);
        if ( ret < 0 )
                return ret;
        ...
        ret = requiemdb_sql_field_to_uint64(field, result);
	if ( ret < 0 )
		goto error;
...
static int insert_message_messageid(requiemdb_sql_t *sql, const char *table_name,
                                    requiem_string_t *messageid, uint64_t *result)
{
	int ret;
        char *tmp;

	ret = requiemdb_sql_escape(sql, get_string(messageid), &tmp);
	if ( ret < 0 )
		return ret;

        ret = requiemdb_sql_insert(sql, table_name, "messageid", "%s", tmp);
	free(tmp);
	if ( ret < 0 )
                return ret;
...
static int insert_alert(requiemdb_sql_t *sql, idmef_alert_t *alert) 
{
        uint64_t ident;
	...
	int ret;
        ...
        ret = insert_message_messageid(sql, "Requiem_Alert", idmef_alert_get_messageid(alert), &ident);
	if ( ret < 0 )
                return ret;
...
static int insert_heartbeat(requiemdb_sql_t *sql, idmef_heartbeat_t *heartbeat) 
{
        uint64_t ident;
        idmef_analyzer_t *analyzer, *last_analyzer;
	...
	int ret;
        ...
        ret = requiemdb_sql_escape(sql, get_string(idmef_heartbeat_get_messageid(heartbeat)), &messageid);
	if ( ret < 0 )
		return ret;
        ...
        ret = requiemdb_sql_insert(sql, "Requiem_Heartbeat", "messageid, heartbeat_interval",
        ...
        ret = get_last_insert_ident(sql, "Requiem_Heartbeat", &ident);
        if ( ret < 0 )
                return ret;
...
int classic_insert(requiemdb_sql_t *sql, idmef_message_t *message)
{
        int ret;

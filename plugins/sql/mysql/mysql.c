...
int mysql_LTX_requiem_plugin_version(void);
int mysql_LTX_requiemdb_plugin_init(requiem_plugin_entry_t *pe, void *data);


static requiem_bool_t is_connection_broken(void *session)
{
...
static int handle_error(void *session, requiem_error_code_t code)
{
        int ret;
        ...
        if ( mysql_errno(session) )
                ret = requiemdb_error_verbose(code, "%s", mysql_error(session));
        else
                ret = requiemdb_error(code);
...
static int sql_open(requiemdb_sql_settings_t *settings, void **session)
{
        ...
        if ( requiemdb_sql_settings_get_port(settings) )
                port = atoi(requiemdb_sql_settings_get_port(settings));
...
static int sql_build_limit_offset_string(void *session, int limit, int offset, requiem_string_t *output)
{
        if ( limit >= 0 ) {
                ...
                return requiem_string_sprintf(output, " LIMIT %d, %" REQUIEM_PRIu64, offset, REQUIEM_UINT64_MAX);
...
static int sql_query(void *session, const char *query, void **resource)
{
        if ( mysql_query(session, query) != 0 )
                return handle_error(session, REQUIEMDB_ERROR_QUERY);
        ...
        *resource = mysql_store_result(session);
        ...
        return mysql_errno(session) ? handle_error(session, REQUIEMDB_ERROR_QUERY) : 0;
}
...
static int sql_fetch_row(void *session, void *resource, void **row)
{
        *row = mysql_fetch_row(resource);
        if ( ! *row )
                return mysql_errno(session) ? requiemdb_error(REQUIEMDB_ERROR_GENERIC) : 0;
                                            ...
                                            idmef_criterion_operator_t operator, int value, int gmt_offset)
{
        ...
        case REQUIEMDB_SQL_TIME_CONSTRAINT_YDAY:
                return requiem_string_sprintf(output, "DAYOFYEAR(%s) %s '%d'",
                                              buf, sql_operator, value);

        case REQUIEMDB_SQL_TIME_CONSTRAINT_MDAY:
                return requiem_string_sprintf(output, "DAYOFMONTH(%s) %s '%d'",
                                              buf, sql_operator, value);

        case REQUIEMDB_SQL_TIME_CONSTRAINT_WDAY:
                return requiem_string_sprintf(output, "DAYOFWEEK(%s) %s '%d'",
...
int mysql_LTX_requiemdb_plugin_init(requiem_plugin_entry_t *pe, void *data)
{
        ...
        requiem_plugin_set_name((requiem_plugin_generic_t *) plugin, "MySQL");

...
int sqlite3_LTX_requiem_plugin_version(void);
int sqlite3_LTX_requiemdb_plugin_init(requiem_plugin_entry_t *pe, void *data);
...
static int sql_open(requiemdb_sql_settings_t *settings, void **session)
{
        ...
        dbfile = requiemdb_sql_settings_get_file(settings);
        if ( ! dbfile || ! *dbfile )
                return requiemdb_error_verbose(REQUIEMDB_ERROR_CONNECTION, "no database file specified");

        ret = access(dbfile, F_OK);
        if ( ret != 0 )
                return requiemdb_error_verbose(REQUIEMDB_ERROR_CONNECTION, "database file '%s' does not exist", dbfile);

        ret = sqlite3_open(dbfile, (sqlite3 **) session);
        if ( ret != SQLITE_OK ) {
                ret = requiemdb_error_verbose(REQUIEMDB_ERROR_CONNECTION, "%s", sqlite3_errmsg(*session));
...
static int sql_build_limit_offset_string(void *session, int limit, int offset, requiem_string_t *output)
{
        if ( limit >= 0 ) {
                if ( offset >= 0 )
                        return requiem_string_sprintf(output, " LIMIT %d, %d", offset, limit);

                return requiem_string_sprintf(output, " LIMIT %d", limit);
        }

        else if ( offset >= 0 )
                return requiem_string_sprintf(output, " LIMIT %d, -1", offset);
...
static sqlite3_row_t *sql_resource_add_row(sqlite3_resource_t *resource, unsigned int cols)
{
        ...
        resource->nrow++;
        requiem_list_add_tail(&resource->rows, &row->list);
...
static void sql_resource_destroy(void *session, void *res)
{
        ...
        requiem_list_t *cursor, *safety_cursor;

        if ( ! resource )
                return;

        requiem_list_for_each_safe(&resource->rows, cursor, safety_cursor) {
                row = requiem_list_entry(cursor, sqlite3_row_t, list);
...
static int sql_query(void *session, const char *query, void **resource)
{
        ...
        if ( strncmp(query, "SELECT", 6) != 0 ) {
                ...
                if ( ret != SQLITE_OK )
                        return requiemdb_error_verbose(REQUIEMDB_ERROR_QUERY, sqlite3_errmsg(session));
...
static int sql_fetch_row(void *session, void *resource, void **row)
{
        ...
        *row = requiem_list_get_next(&res->rows, res->current_row, sqlite3_row_t, list);
        if ( ! *row ) {
                                       ...
                                       idmef_criterion_operator_t operator, const char *value)
{
        ...
        if ( operator & IDMEF_CRITERION_OPERATOR_NOCASE )
                return requiem_string_sprintf(out, "lower(%s) %s lower(%s)", field, op_str, value);
                                            ...
                                            idmef_criterion_operator_t operator, int value, int gmt_offset)
{
        ...
        switch ( type ) {
                case REQUIEMDB_SQL_TIME_CONSTRAINT_YEAR:
                        return requiem_string_sprintf(output, "STRFTIME('%%Y', %s) + 0 %s %d",
                                        buf, sql_operator, value);

                case REQUIEMDB_SQL_TIME_CONSTRAINT_MONTH:
                        return  requiem_string_sprintf(output, "STRFTIME('%%m', %s) + 0 %s %d",
                                        buf, sql_operator, value);

                case REQUIEMDB_SQL_TIME_CONSTRAINT_YDAY:
                        return requiem_string_sprintf(output, "STRFTIME('%%j', %s) + 0 %s %d",
                                        buf, sql_operator, value);

                case REQUIEMDB_SQL_TIME_CONSTRAINT_MDAY:
                        return requiem_string_sprintf(output, "STRFTIME('%%d', %s) + 0 %s %d",
                                        buf, sql_operator, value);

                case REQUIEMDB_SQL_TIME_CONSTRAINT_WDAY:
                        return requiem_string_sprintf(output, "STRFTIME('%%w', %s) + 0 %s %d",
                                        buf, sql_operator, value % 7);

                case REQUIEMDB_SQL_TIME_CONSTRAINT_HOUR:
                        return requiem_string_sprintf(output, "STRFTIME('%%H', %s) + 0 %s %d",
                                        buf, sql_operator, value);
                ...
                case REQUIEMDB_SQL_TIME_CONSTRAINT_SEC:
                        return requiem_string_sprintf(output, "STRFTIME('%%S', %s) + 0 %s %d",
                                        buf, sql_operator, value);
        }
...
int sqlite3_LTX_requiemdb_plugin_init(requiem_plugin_entry_t *pe, void *data)
{
        ...
        requiem_plugin_set_name((requiem_plugin_generic_t *) plugin, "sqlite3");

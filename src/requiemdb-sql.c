...
#include <librequiem/requiem-linked-object.h>
...
typedef enum {
        REQUIEMDB_SQL_STATUS_CONNECTED    = 0x01,
        REQUIEMDB_SQL_STATUS_TRANSACTION  = 0x02
} requiemdb_sql_status_t;



#define assert_connected(sql)                                             \
        if ( ! (sql->status & REQUIEMDB_SQL_STATUS_CONNECTED) ) {         \
                int __ret;                                                \
                                                                          \
                __ret = requiemdb_sql_connect(sql);                       \
                if ( __ret < 0 ) {                                        \
...
struct requiemdb_sql {
        char *type;
        requiemdb_sql_settings_t *settings;
        requiemdb_plugin_sql_t *plugin;
        requiemdb_sql_status_t status;
        void *session;
        FILE *logfile;
        requiem_bool_t internal_transaction_disabled;

        gl_recursive_lock_t mutex;
};

struct requiemdb_sql_table {
        requiemdb_sql_t *sql;
        void *res;
        requiem_list_t row_list;
};

struct requiemdb_sql_row {
        requiem_list_t list;
        requiemdb_sql_table_t *table;
        void *res;
        requiem_list_t field_list;
};

struct requiemdb_sql_field {
        requiem_list_t list;
...
extern requiem_list_t _sql_plugin_list;




static inline void update_sql_from_errno(requiemdb_sql_t *sql, requiemdb_error_t error)
{
        if ( requiemdb_error_check(error, REQUIEMDB_ERROR_CONNECTION) ) {
                _requiemdb_plugin_sql_close(sql->plugin, sql->session);
                sql->status &= ~REQUIEMDB_SQL_STATUS_CONNECTED;
        }
}
 ...
 */
int requiemdb_sql_new(requiemdb_sql_t **new, const char *type, requiemdb_sql_settings_t *settings)
{
        *new = calloc(1, sizeof(**new));
        ...
        if ( ! type ) {
                type = requiemdb_sql_settings_get_type(settings);
                if ( ! type )
                        return requiemdb_error_verbose(REQUIEMDB_ERROR_INVALID_SETTINGS_STRING, "No database type specified");
        }
        ...
        (*new)->plugin = (requiemdb_plugin_sql_t *) requiem_plugin_search_by_name(&_sql_plugin_list, type);
        if ( ! (*new)->plugin ) {
                free((*new)->type);
                free(*new);
                return requiemdb_error_verbose(REQUIEMDB_ERROR_CANNOT_LOAD_SQL_PLUGIN, "Could not load sql plugin '%s'", type);
        }

        if ( requiemdb_sql_settings_get_log(settings) )
                requiemdb_sql_enable_query_logging(*new, requiemdb_sql_settings_get_log(settings));
...
void requiemdb_sql_destroy(requiemdb_sql_t *sql)
{
        ...
        requiemdb_sql_settings_destroy(sql->settings);
...
/**
 * requiemdb_sql_enable_query_logging:
 * @sql: Pointer to a sql object.
...
int requiemdb_sql_enable_query_logging(requiemdb_sql_t *sql, const char *filename)
{
        ...
        if ( ! sql->logfile )
                return requiemdb_error_verbose(requiem_error_code_from_errno(errno),
...
/**
 * requiemdb_sql_disable_query_logging:
 * @sql: Pointer to a sql object.
 *
...
static int requiemdb_sql_connect(requiemdb_sql_t *sql)
{
        int ret;

        ret = _requiemdb_plugin_sql_open(sql->plugin, sql->settings, &sql->session);
        if ( ret < 0 )
                return ret;
...
static int requiemdb_sql_table_new(requiemdb_sql_table_t **new, requiemdb_sql_t *sql, void *res)
{
        *new = malloc(sizeof(**new));
        ...
        (*new)->sql = sql;
        (*new)->res = res;
        requiem_list_init(&(*new)->row_list);
...
int requiemdb_sql_query(requiemdb_sql_t *sql, const char *query, requiemdb_sql_table_t **table)
{
        ...
        ret = _requiemdb_plugin_sql_query(sql->plugin, sql->session, query, &res);
        ...
        ret = requiemdb_sql_table_new(table, sql, res);
        if ( ret < 0 ) {
                _requiemdb_plugin_sql_resource_destroy(sql->plugin, sql->session, res);
                return ret;
        }

        return requiemdb_sql_table_get_row_count(*table);
}



/**
 * requiemdb_sql_query_sprintf:
 * @sql: Pointer to a sql object.
 * @table: Pointer to a table where the query result will be stored if the type of query return
 * results (i.e a SELECT can results, but an INSERT never results) and if the query is sucessfull.
 ...
 */
int requiemdb_sql_query_sprintf(requiemdb_sql_t *sql, requiemdb_sql_table_t **table,
                                const char *format, ...)
{
        int ret;
        va_list ap;
        ...
        va_start(ap, format);
        ret = requiem_string_vprintf(query, format, ap);
        va_end(ap);
 ...
 * Returns: 0 on success or a negative value if an error occur.
 */
int requiemdb_sql_insert(requiemdb_sql_t *sql, const char *table, const char *fields,
                         const char *format, ...)
{
        int ret;
        va_list ap;
        ...
        ret = requiem_string_sprintf(query, "INSERT INTO %s (%s) VALUES(", table, fields);
        if ( ret < 0 )
                goto error;
        ...
        ret = requiem_string_cat(query, ")");
        if ( ret < 0 )
                goto error;

        ret = requiemdb_sql_query(sql, requiem_string_get_string(query), NULL);
...
/**
 * requiemdb_sql_build_limit_offset_string:
 * @sql: Pointer to a sql object.
...
{
        return _requiemdb_plugin_sql_build_limit_offset_string(sql->plugin, sql->session, limit, offset, output);
}
...
int _requiemdb_sql_transaction_start(requiemdb_sql_t *sql)
{
        ...
        if ( sql->status & REQUIEMDB_SQL_STATUS_TRANSACTION ) {
                gl_recursive_lock_unlock(sql->mutex);
                return requiemdb_error(REQUIEMDB_ERROR_ALREADY_IN_TRANSACTION);
        }

        ret = requiemdb_sql_query(sql, "BEGIN", NULL);
        if ( ret < 0 )
                gl_recursive_lock_unlock(sql->mutex);
        else
                sql->status |= REQUIEMDB_SQL_STATUS_TRANSACTION;
...
/**
 * requiemdb_sql_transaction_start:
 * @sql: Pointer to a sql object.
 *
...
int requiemdb_sql_transaction_start(requiemdb_sql_t *sql)
{
        ...
        return _requiemdb_sql_transaction_start(sql);
}
...
int _requiemdb_sql_transaction_end(requiemdb_sql_t *sql)
{
        ...
        if ( ! (sql->status & REQUIEMDB_SQL_STATUS_TRANSACTION) )
                return requiemdb_error(REQUIEMDB_ERROR_NOT_IN_TRANSACTION);

        ret = requiemdb_sql_query(sql, "COMMIT", NULL);
        sql->status &= ~REQUIEMDB_SQL_STATUS_TRANSACTION;
...
int requiemdb_sql_transaction_end(requiemdb_sql_t *sql)
{
        ...
        return _requiemdb_sql_transaction_end(sql);
}
...
int _requiemdb_sql_transaction_abort(requiemdb_sql_t *sql)
{
        ...
        if ( _requiem_thread_get_error() )
                original_error = strdup(_requiem_thread_get_error());

        sql->status &= ~REQUIEMDB_SQL_STATUS_TRANSACTION;

        if ( original_error && ! (sql->status & REQUIEMDB_SQL_STATUS_CONNECTED) ) {
                ret = requiemdb_error_verbose(REQUIEMDB_ERROR_QUERY, "%s. No ROLLBACK possible due to connection closure",
                                              original_error);
                goto error;
        }

        ret = requiemdb_sql_query(sql, "ROLLBACK", NULL);
        if ( ret < 0 ) {
                if ( original_error )
                        ret = requiemdb_error_verbose(REQUIEMDB_ERROR_QUERY, "%s.\nROLLBACK failed: %s", original_error, requiemdb_strerror(ret));
                else
                        ret = requiemdb_error_verbose(REQUIEMDB_ERROR_QUERY, "ROLLBACK failed: %s", requiemdb_strerror(ret));
        }
...
/**
 * requiemdb_sql_transaction_abort:
 * @sql: Pointer to a sql object.
 *
...
int requiemdb_sql_transaction_abort(requiemdb_sql_t *sql)
{
        ...
        return _requiemdb_sql_transaction_abort(sql);
}



/**
 * requiemdb_sql_escape_fast:
 * @sql: Pointer to a sql object.
...
int requiemdb_sql_escape_fast(requiemdb_sql_t *sql, const char *input, size_t input_size, char **output)
{
        ...
        assert_connected(sql);
        ret = _requiemdb_plugin_sql_escape(sql->plugin, sql->session, input, input_size, output);
...
int requiemdb_sql_escape(requiemdb_sql_t *sql, const char *input, char **output)
{
        if ( ! input ) {
                *output = strdup("NULL");
                return *output ? 0 : requiemdb_error_from_errno(errno);
        }

        return requiemdb_sql_escape_fast(sql, input, strlen(input), output);
}



/**
 * requiemdb_sql_escape_binary:
 * @sql: Pointer to a sql object.
 * @input: Buffer to escape.
 * @input_size: Buffer size.
 * @output: Where the new escaped buffer will be stored.
 ...
 * Returns: 0 on success or a negative value if an error occur.
 */
int requiemdb_sql_escape_binary(requiemdb_sql_t *sql, const unsigned char *input, size_t input_size,
                                char **output)
{
        int ret;
        ...
        assert_connected(sql);
        ret = _requiemdb_plugin_sql_escape_binary(sql->plugin, sql->session, input, input_size, output);
                                  ...
                                  unsigned char **output, size_t *output_size)
{
        ...
        assert_connected(sql);
        ret = _requiemdb_plugin_sql_unescape_binary(sql->plugin, sql->session, input, input_size, output, output_size);
...
{
        requiemdb_sql_t *sql = table->sql;
        requiemdb_sql_row_t *row;
        requiemdb_sql_field_t *field;
        requiem_list_t *tmp, *next_row, *next_field;

        requiem_list_for_each_safe(&table->row_list, tmp, next_row) {
                row = requiem_list_entry(tmp, requiemdb_sql_row_t, list);

                requiem_list_for_each_safe(&row->field_list, tmp, next_field) {
                        field = requiem_list_entry(tmp, requiemdb_sql_field_t, list);
                        free(field);
                }
        ...
        _requiemdb_plugin_sql_resource_destroy(sql->plugin, sql->session, table->res);
        free(table);
}



static int requiemdb_sql_row_new(requiemdb_sql_row_t **row, requiemdb_sql_table_t *table, void *res)
{
        *row = malloc(sizeof(**row));
        ...
        requiem_list_init(&(*row)->list);
        requiem_list_init(&(*row)->field_list);

        requiem_list_add_tail(&table->row_list, &(*row)->list);
...
static int requiemdb_sql_field_new(requiemdb_sql_field_t **field,
                                   requiemdb_sql_row_t *row, int num, const char *value, size_t len)
{
        *field = malloc(sizeof(**field));
        ...
        requiem_list_init(&(*field)->list);

        requiem_list_add_tail(&row->field_list, &(*field)->list);
...
/**
 * requiemdb_sql_table_get_column_name:
 * @table: Pointer to a table object.
...
{
        return _requiemdb_plugin_sql_get_column_name(table->sql->plugin, table->sql->session, table->res, column_num);
}



/**
 * requiemdb_sql_table_get_column_num:
 * @table: Pointer to a table object.
...
{
        return _requiemdb_plugin_sql_get_column_num(table->sql->plugin, table->sql->session, table->res, column_name);
}



/**
 * requiemdb_sql_table_get_column_count:
 * @table: Pointer to a table object.
 *
...
{
        return _requiemdb_plugin_sql_get_column_count(table->sql->plugin, table->sql->session, table->res);
}
...
{
        return _requiemdb_plugin_sql_get_row_count(table->sql->plugin, table->sql->session, table->res);
}



/**
 * requiemdb_sql_table_fetch_row:
 * @table: Pointer to a table object.
...
int requiemdb_sql_table_fetch_row(requiemdb_sql_table_t *table, requiemdb_sql_row_t **row)
{
        ...
        ret = _requiemdb_plugin_sql_fetch_row(table->sql->plugin, table->sql->session, table->res, &res);
        if ( ret < 0 ) {
        ...
        ret = requiemdb_sql_row_new(row, table, res);
        if ( ret < 0 )
                return ret;
 ...
 * a negative value if an error occur.
 */
int requiemdb_sql_row_fetch_field(requiemdb_sql_row_t *row, unsigned int column_num,
                                  requiemdb_sql_field_t **field)
{
        const char *value;
        size_t len;
        ...
        ret = _requiemdb_plugin_sql_fetch_field(row->table->sql->plugin,
        ...
        ret = requiemdb_sql_field_new(field, row, column_num, value, len);
        if ( ret < 0 )
                return ret;
...
/**
 * requiemdb_sql_row_fetch_field_by_name:
 * @row: Pointer to a row object.
 ...
 * a negative value if an error occur.
 */
int requiemdb_sql_row_fetch_field_by_name(requiemdb_sql_row_t *row, const char *column_name,
                                          requiemdb_sql_field_t **field)
{
        int column_num;

        column_num = requiemdb_sql_table_get_column_num(row->table, column_name);
        if ( column_num < 0 )
                return column_num;

        return requiemdb_sql_row_fetch_field(row, column_num, field);
}



/**
 * requiemdb_sql_field_get_value:
...
/**
 * requiemdb_sql_field_to_{int8,uint8,int16,uint16,int32,uint32,int64,uint64,float,double}:
 * @field: Pointer to a field object.
 ...
 */
#define requiemdb_sql_field_to(name, type_name, format)                                         \
int requiemdb_sql_field_to_ ## name(requiemdb_sql_field_t *field, type_name *value)             \
{                                                                                               \
        int ret;                                                                                \
                                                                                                \
        ret = sscanf(requiemdb_sql_field_get_value(field), format, value);                      \
        if ( ret < 0 )                                                                          \
                return requiemdb_error(REQUIEMDB_ERROR_INVALID_VALUE);                          \
                                                                                                \
        return 0;                                                                               \
}

#define requiemdb_sql_field_to_int8(name, type_name, format, min, max)                          \
int requiemdb_sql_field_to_ ## name(requiemdb_sql_field_t *field, type_name *value)             \
{                                                                                               \
        int tmp, ret;                                                                           \
                                                                                                \
        ret = sscanf(requiemdb_sql_field_get_value(field), format, &tmp);                       \
 ...
 */
requiemdb_sql_field_to_int8(int8, int8_t, "%d", REQUIEM_INT8_MIN, REQUIEM_INT8_MAX)
requiemdb_sql_field_to_int8(uint8, uint8_t, "%u", 0, REQUIEM_UINT8_MAX)
requiemdb_sql_field_to(int16, int16_t, "%hd")
requiemdb_sql_field_to(uint16, uint16_t, "%hu")
requiemdb_sql_field_to(int32, int32_t, "%d")
requiemdb_sql_field_to(uint32, uint32_t, "%u")
requiemdb_sql_field_to(int64, int64_t, "%" REQUIEM_PRId64)
requiemdb_sql_field_to(uint64, uint64_t, "%" REQUIEM_PRIu64)
requiemdb_sql_field_to(float, float, "%f")
requiemdb_sql_field_to(double, double, "%lf")



/**
 * requiemdb_sql_field_to_string:
...
{
        return requiem_string_ncat(output, field->value, field->len);
}


static int build_criterion_fixed_sql_time_value(requiemdb_sql_t *sql,
                                                const idmef_value_t *value, char *buf, size_t size)
{
        ...
        return requiemdb_sql_time_to_timestamp(sql, time, buf, size, NULL, 0, NULL, 0);
}
...
static int build_criterion_fixed_sql_like_value(const idmef_value_t *value, char **output)
{
        ...
        const char *input;
        idmef_data_t *data;
        requiem_string_t *outbuf;
        const requiem_string_t *string;
        requiem_bool_t escape_next = FALSE;
        ...
        if ( idmef_value_get_type(value) == IDMEF_VALUE_TYPE_DATA ) {
                ...
                input = requiem_string_get_string(string);
                if ( ! input )
                        return -1;

                len = requiem_string_get_len(string);
        }

        ret = requiem_string_new(&outbuf);
        if ( ret < 0 )
                return ret;
        ...
        for ( i = 0; i < len; i++ ) {
                ...
                else requiem_string_ncat(outbuf, input, 1);

                escape_next = (! escape_next && *input == '\\') ? TRUE : FALSE;
                input++;
        }

        ret = requiem_string_get_string_released(outbuf, output);
        requiem_string_destroy(outbuf);
...
static int build_criterion_fixed_sql_value(requiemdb_sql_t *sql,
                                           ...
                                           idmef_criterion_operator_t operator)
{
        ...
        if ( idmef_value_get_type(value) == IDMEF_VALUE_TYPE_TIME ) {
                char buf[REQUIEMDB_SQL_TIMESTAMP_STRING_SIZE];

                ret = build_criterion_fixed_sql_time_value(sql, value, buf, sizeof(buf));
                if ( ret < 0 )
                        return ret;

                return requiem_string_cat(output, buf);
        }
        ...
        if ( operator & IDMEF_CRITERION_OPERATOR_SUBSTR ) {
                ...
                ret = requiemdb_sql_escape(sql, tmp, &escaped);
                if ( ret < 0 ) {
                        free(tmp);
                        return ret;
                }

                ret = requiem_string_cat(output, escaped);
        ...
        ret = requiemdb_sql_escape(sql, requiem_string_get_string(string), &escaped);
        requiem_string_destroy(string);
        if ( ret < 0 )
                return ret;
...
static int build_criterion_fixed_value(requiemdb_sql_t *sql,
                                       requiem_string_t *output,
                                       const char *field,
                                       idmef_criterion_operator_t operator,
                                       const idmef_value_t *value)
{
        int ret;
        requiem_string_t *value_str;

        ret = requiem_string_new(&value_str);
        if ( ret < 0 )
                return ret;
        ...
        ret = _requiemdb_plugin_sql_build_constraint_string(sql->plugin, output, field, operator, requiem_string_get_string(value_str));

 err:
        requiem_string_destroy(value_str);
        return ret;
}
...
static int build_criterion_broken_down_time_equal(requiemdb_sql_t *sql, requiem_string_t *output,
                                                  const char *field, idmef_criterion_operator_t op, const struct tm *lt)
{
        ...
        const struct {
                ...
                { REQUIEMDB_SQL_TIME_CONSTRAINT_YEAR, &year        },
                { REQUIEMDB_SQL_TIME_CONSTRAINT_MONTH, &month      },
                { REQUIEMDB_SQL_TIME_CONSTRAINT_YDAY, &lt->tm_yday },
                { REQUIEMDB_SQL_TIME_CONSTRAINT_MDAY, &lt->tm_mday },
                { REQUIEMDB_SQL_TIME_CONSTRAINT_WDAY, &lt->tm_wday },
                { REQUIEMDB_SQL_TIME_CONSTRAINT_HOUR, &lt->tm_hour },
                { REQUIEMDB_SQL_TIME_CONSTRAINT_MIN,  &lt->tm_min  },
                { REQUIEMDB_SQL_TIME_CONSTRAINT_SEC,  &lt->tm_sec  }
        };
        ...
        for ( i = 0; i < sizeof(tbl) / sizeof(*tbl); i++ ) {
                ...
                ret = _requiemdb_plugin_sql_build_time_constraint_string(sql->plugin, output, field, tbl[i].type,
...
static int build_criterion_broken_down_time_not_equal(requiemdb_sql_t *sql, requiem_string_t *output,
                                                      const char *field, const struct tm *lt)
{
        int ret;

        ret = requiem_string_cat(output, "NOT(");
        if ( ret < 0 )
                return ret;
...
static int build_criterion_broken_down_time(requiemdb_sql_t *sql, requiem_string_t *output,
...
static int build_criterion_regex(requiemdb_sql_t *sql, requiem_string_t *output,
                                 const char *field, idmef_criterion_operator_t op, const char *regex)
{
        int ret;
        char *escaped;

        ret = requiemdb_sql_escape(sql, regex, &escaped);
        if ( ret < 0 )
                return ret;

        ret = _requiemdb_plugin_sql_build_constraint_string(sql->plugin, output, field, op, escaped);
        free(escaped);
...
/**
 * requiemdb_sql_build_criterion_string:
 * @sql: Pointer to a sql object.
                                         ...
                                         idmef_criterion_operator_t operator, idmef_criterion_value_t *value)
{
        ...
        if ( operator == IDMEF_CRITERION_OPERATOR_NULL )
                return requiem_string_sprintf(output, "%s %s", field,
                                              _requiemdb_plugin_sql_get_operator_string(sql->plugin, operator));
                ...
                ret = requiem_string_sprintf(output, "(%s %s OR ", field,
                             _requiemdb_plugin_sql_get_operator_string(sql->plugin, IDMEF_CRITERION_OPERATOR_NULL));
                if ( ret < 0 )
                        return ret;
        }
        ...
        if ( ret >= 0 && operator & IDMEF_CRITERION_OPERATOR_NOT )
                ret = requiem_string_sprintf(output, ")");
...
/**
 * requiemdb_sql_time_from_timestamp:
...
int requiemdb_sql_time_from_timestamp(idmef_time_t *time, const char *time_buf, int32_t gmtoff, uint32_t usec)
{
        ...
        if ( ret < 6 )
                return requiemdb_error_verbose(REQUIEMDB_ERROR_GENERIC, "Database returned an unknown time format: '%s'", time_buf);

        tm.tm_year -= 1900;
        tm.tm_mon -= 1;
        tm.tm_isdst = -1;

        idmef_time_set_sec(time, requiem_timegm(&tm));
...
/**
 * requiemdb_sql_time_to_timestamp:
 * @time: Pointer to a time object.
 * @time_buf: SQL timestamp.
                                    ...
                                    char *usec_buf, size_t usec_buf_size)
{
        ...
        ret = _requiemdb_plugin_sql_build_timestamp_string(sql->plugin, &utc, time_buf, time_buf_size);
        if ( ret < 0 )
                return ret;
...
/**
 * requiemdb_sql_get_plugin_error:
 * @sql: Pointer to a sql object.
 *
